...
class QuantrupedHierarchicalEnv(QuantrupedHierarchicalControllerSuperEnv):
    ...
    def __init__(self, config):
        ...
        self.obs_indices["high_level"] = [45, ]  # range(46)
    ...
    def return_policies():
        ...
        obs_space = spaces.Box(-np.inf, np.inf, (1,), np.float64)  # 15
        ...
        policies = {
            ...
            QuantrupedHierarchicalEnv.policy_names[1]: (None,
                                                        spaces.Tuple([LL_obs_space, spaces.Box(np.array([-1., ]*latent_space_len), np.array([+1., ]*latent_space_len))]), spaces.Box(np.array([-1., -1., -1., -1., -1., -1., -1., -1.]), np.array([+1., +1., +1., +1., +1., +1., +1., +1.])), {"model": {"custom_model": "llc_glorot_uniform_init", 'fcnet_hiddens': [150, 150, 150], }}),
        }
        """
        policies = {
            QuantrupedHierarchicalEnv.policy_names[0]: (None,
                                                        obs_space, spaces.Box(np.array([-1, ]*latent_space_len), np.array([+1, ]*latent_space_len)), {"model": {"custom_model": "fc_glorot_uniform_init", 'fcnet_hiddens': [64, 64, ], }}),
    ...
    def reset(self):
        ...
        return {
            "high_level_policy": self.cur_obs[self.obs_indices["high_level"]],
        }
    ...
    def _low_level_step(self, action):
        ...
        if (distance_rob_target < 0.25):
            ...
            rew["high_level_policy"] += self.target_weight * \
                (self.target_reward - self.env_step_counter)
            obs["high_level_policy"] = self.cur_obs[self.obs_indices["high_level"]]
...
class QuantrupedHierarchicalFullyDecentralizedEnv(QuantrupedHierarchicalControllerSuperEnv):
    ...
    def __init__(self, config):
        ...
        # [0, 1, 2, 3, 4, 5, 6, 15, 16, 17, 18, 19, 20, 45, 46]
        self.obs_indices["high_level"] = [45, ]  # range(47)
    ...
    def return_policies():
        ...
        obs_space = spaces.Box(-np.inf, np.inf, (1,), np.float64)
    ...
    def _low_level_step(self, action_dict):
        ...
        rew = {"low_level_FL_policy": rew_val[self.policy_names[1]], "low_level_HL_policy": rew_val[self.policy_names[2]],
               "low_level_HR_policy": rew_val[self.policy_names[3]], "low_level_FR_policy": rew_val[self.policy_names[4]]}
