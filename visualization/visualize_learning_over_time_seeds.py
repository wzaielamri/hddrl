...
exp_path = [
    '/media/compute/homes/wzaielamri/ray_results/0_2_0_velocity_DifferentAngleFarTarget_10Steps_LS8_LLCOnlyLocalInfo_NoLSTM_QuantrupedMultiEnv_Hierarchical',
    '/media/compute/homes/wzaielamri/ray_results/0_3_0_velocity_DifferentAngleFarTarget_10Steps_LS8-split_NoLSTM_QuantrupedMultiEnv_HierarchicalFullyDecentralized'
]
...
all_exp_data = []
time_steps = []
for ind, exp_dir in enumerate(experiment_dirs):
    for i in range(0, len(exp_dir)):
        df = pd.read_csv(exp_dir[i]+'/progress.csv')
        rew_new = (df.iloc[:, column[ind]].values)
        if i == 0:
...
for i in range(0, len(all_exp_data)):
    ...
    plt.plot(time_steps[i], all_exp_data[i][0],
             color=tableau20[i*2], lw=1, label=exp_path[i].split('/')[-1].split("_")[-1])
