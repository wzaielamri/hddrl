...
class QuantrupedMultiPoliciesEnv(MultiAgentEnv):
    ...
    def __init__(self, config):
        ...
        if 'target_weight' in config.keys():
        ...
        self.env = gym.make("QuAntrupedSeek-v3",
    ...
    def step(self, action_dict):
        ...
        if (distance_rob_target < 0.25):
            ...
            for policy in list(action_dict.keys()):
                rew_dict[policy] = (self.target_weight*(
                    1-(self.env_step_counter/self.target_reward)))/len(list(action_dict.keys()))
        else:
            for policy in list(action_dict.keys()):
                rew_dict[policy] = 0
