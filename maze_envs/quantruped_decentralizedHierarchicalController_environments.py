...
class QuantrupedFullyDecentralizedHierarchicalEnv(QuantrupedHierarchicalControllerSuperEnv):
    ...
    def return_policies():
        ...
        policies = {
            QuantrupedFullyDecentralizedHierarchicalEnv.policy_names[0]: (None,
                                                                          obs_space, spaces.Box(np.array([-1, ]*2), np.array([+1, ]*2)), {"model": {"custom_model": "fc_glorot_uniform_init_lstm", 'fcnet_hiddens': [64, 64, ], "lstm_cell_size": 64}}),
    ...
    def reset(self):
        ...
        obs_hlc = np.concatenate(
            [maze_obs, self.cur_obs[self.obs_indices["high_level_FL_policy"]]])
    ...
    def _low_level_step(self, action):
        ...
        if (self.env.is_in_first_goal((obs_full[0], obs_full[1]))):
            rew["high_level_FL_policy"] = 1/4
            rew["high_level_HL_policy"] = 1/4
            rew["high_level_HR_policy"] = 1/4
            rew["high_level_FR_policy"] = 1/4
            obs["high_level_FL_policy"] = self.cur_obs[self.obs_indices["high_level_FL_policy"]]
            # obs_hlc
            obs["high_level_HL_policy"] = self.cur_obs[self.obs_indices["high_level_HL_policy"]]
            # obs_hlc
        ...
        if (self.env.is_in_goal((obs_full[0], obs_full[1]))):
            ...
            reward_hlc = 1#7*(1-(self.env_step_counter/self.target_reward))
